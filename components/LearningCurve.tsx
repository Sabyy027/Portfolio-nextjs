  ...
  const NodeCard = ({ node, index, isVertical = false }: { node: LearningNode, index: number, isVertical?: boolean }) => {
    ...
    return (
        <div className="min-w-[400px] snap-center px-4 relative group h-full">
            {/* Center Line Dot */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-4 rounded-full bg-zinc-950 border-2 border-zinc-700 group-hover:border-green-500 z-20 transition-colors shadow-lg">
                <div className={`w-1.5 h-1.5 rounded-full ${bg} absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2`}></div>
            </div>

            <div className={`flex flex-col h-full ${isTop ? 'justify-end pb-12' : 'justify-start pt-12'}`}>
                {/* Connector Line */}
                <div className={`absolute left-1/2 w-[1px] bg-zinc-800 group-hover:bg-zinc-600 transition-colors -z-10 ${isTop ? 'bottom-[50%] h-12' : 'top-[50%] h-12'}`}></div>
                ...
                <motion.div 
                    ...
                    viewport={{ once: true }}
                    className="bg-zinc-900/40 p-6 rounded-2xl border border-white/5 hover:border-white/10 hover:shadow-2xl transition-all backdrop-blur-sm relative group-hover:-translate-y-1 duration-300 w-full"
                >
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="hidden md:block relative h-[600px]">
           ...
           <div 
             ...
             className="flex overflow-x-auto gap-0 h-full hide-scrollbar snap-x items-center px-8"
